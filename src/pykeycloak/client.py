...
from pydantic import BaseModel, SecretStr, FilePath, HttpUrl, validate_arguments, parse_obj_as
...
class TokenFileContent(BaseModel):
    ...
    def to_json(self) -> dict:
        return {
            ...
            'token_timestamp': self.token_timestamp.isoformat(),
...
class Client(object):
    ...
    async def get_token_timestamp(self) -> datetime:
        '''
    ...
    async def get_access_token_expiry_timestamp(self) -> Union[datetime, None]:
        '''
    ...
    async def get_refresh_token_expiry_timestamp(self) -> Union[datetime, None]:
        '''
    ...
    async def get_access_token(self) -> str:
        with self.__lock:
            token_file_contents = await self.__get_token_attributes()
            now = datetime.now()
            access_token_expiry = token_file_contents.token_timestamp
            if token_file_contents.access_token_lifespan < 0:
                warnings.warn('We do not know if the access token has expired or not.')
    ...
    async def get_refresh_token(self) -> Union[str, None]:
        with self.__lock:
            ...
            if token_file_contents.refresh_token_lifespan >= 0 and now > refresh_token_expiry:
